package team.gravityrecode.clientbase.impl.util.foint;
...
public class MCFontRenderer extends CFont {
    ...
    public float drawStringWithShadow(String text, float x, float y, int color) {
        float shadowWidth = drawString(text, x + .5D, y + .5D, color, true);
        return Math.max(shadowWidth, drawString(text, x, y, color, false));
    }

    public float drawStringWithShadow(String text, double x, double y, int color) {
        float shadowWidth = drawString(text, x + .5, y + .5, color, true);
        return Math.max(shadowWidth, drawString(text, x, y, color, false));
    }
    ...
    public float drawString(String text, double x, double y, int color, boolean shadow) {
        ...
        if (shadow) {
            drawString(text,x+.5,y+.5,(color & 0xFCFCFC) >> 2 | color & new Color(20, 20, 20, 200).getRGB());
        }
