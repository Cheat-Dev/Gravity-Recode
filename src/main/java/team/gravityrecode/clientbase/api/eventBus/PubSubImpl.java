        ...
        for (Method method : subscriber.getClass().getDeclaredMethods()) {
            ...
            if (method.isAnnotationPresent(EventHandler.class)) {
                ...
                if (!Modifier.isPublic(mm)) {
                    this.error(String.format("ERROR: %s#%s: Methods marked with `@EventHandler` must be public.",
                            method.getDeclaringClass().getSimpleName(), method.getName()));
                    continue;
                }
                // check is non-static
                if (Modifier.isStatic(mm)) {
                    this.error(String.format("ERROR: %s#%s: Methods marked with `@EventHandler` must be non-static.",
                            method.getDeclaringClass().getSimpleName(), method.getName()));
                    continue;
                }
                // check is non-abstract
                if (Modifier.isAbstract(mm)) {
                    this.error(String.format("ERROR: %s#%s: Methods marked with `@EventHandler` must be non-abstract.",
                            method.getDeclaringClass().getSimpleName(), method.getName()));
                    continue;
                }
                // check is non-native
                if (Modifier.isNative(mm)) {
                    this.error(String.format("ERROR: %s#%s: Methods marked with `@EventHandler` must be non-native.",
                            method.getDeclaringClass().getSimpleName(), method.getName()));
                    continue;
                }
                // check method signature
                if (method.getReturnType() != void.class) {
                    this.error(String.format("ERROR: %s#%s: Methods marked with `@EventHandler` must return `void`.",
                            method.getDeclaringClass().getSimpleName(), method.getName()));
                    continue;
                }
                ...
                if (paramTypes.length != 1) {
                    this.error(String.format("ERROR: %s#%s: methods marked with `@EventHandler` must have only a single parameter.",
                            method.getDeclaringClass().getSimpleName(), method.getName()));
                    continue;
                }
                ...
                try {
                    ...
                    this.error(String.format("ERROR: %s#%s: the parameter of methods marked `@EventHandler` must be a subclass of the PubSub<Event> `Event` type parameter.",
                            method.getDeclaringClass().getSimpleName(), method.getName()));
                } catch (Throwable e) {
